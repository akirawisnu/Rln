...
class RlnGuiApp:
    ...
    def _build_script_pane(self) -> None:
        ...
        header.pack(fill="x")
        ttk.Label(header, text="Script", font=("TkDefaultFont", 10, "bold")).pack(side="left", padx=6, pady=4)
    ...
    def run_script(self) -> None:
        ...
        if not script.strip():
            return
        self.notebook.select(0)
        tmp = tempfile.NamedTemporaryFile("w", suffix=".rln", encoding="utf-8", delete=False)
        try:
            tmp.write(script)
            tmp_path = tmp.name
        finally:
