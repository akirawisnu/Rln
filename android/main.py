...
a tabbed notebook (Console / Variables / Data / Plots / Script / Help), a
floating circular menu button, and a script editor. Tkinter cannot run
...
ABOUT_TEXT = (
    ...
    "versions, so your scripts behave identically across platforms.\n\n"
    ...
    "the Script tab and press Run script.\n\n"
...
if HAVE_CODEINPUT:
    ...
    class RlnLexer(RegexLexer):
        """Minimal lexer for the Rln command language (Rln command language)."""
        name = "Rln"
        aliases = ["rln"]
        tokens = {
            "root": [
...
def ensure_storage_access():
    ...
    exposes media, not .csv/.dta/.rln script files. The fix is "All files access"
...
def run_do_file(filepath, parser, console):
    """Execute a script file. Ported verbatim from the desktop CLI (main.py) so the
...
class RlnApp(App):
    ...
    def build(self):
        ...
        # resizable Script editor at the bottom (mirrors the desktop
    ...
    def _build_script_pane(self):
        ...
        # matching the desktop "Script" pane.
        box = BoxLayout(orientation="vertical", spacing=dp(4),
                        padding=(dp(2), dp(2), dp(2), 0))
        row = BoxLayout(size_hint_y=None, height=dp(38), spacing=dp(5))
        lab = Label(text="[b]Script[/b]", markup=True, halign="left",
        ...
        box.add_widget(row)
        hint = "Write a multi-line script file here, then Run script..."
        if HAVE_CODEINPUT:
    ...
    def run_script(self):
        ...
        tmp = tempfile.NamedTemporaryFile(
            "w", suffix=".rln", encoding="utf-8", delete=False,
            dir=writable_dir())
        try:
            tmp.write(script)
            path = tmp.name
    ...
    def open_script(self):
        ...
        def on_pick(path):
            try:
                ...
                self._toast(str(exc))
        self._file_dialog("Open script file", on_pick,
                          filters=["*.rln", "*.do", "*.txt", "*"])
    ...
    def save_script(self):
        def on_name(name):
            ...
            try:
                ...
                self._toast(str(exc))
        self._name_dialog("Save script file as", "script.rln", on_name)
