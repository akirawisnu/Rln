...
class KivyRecipe(PyProjectRecipe):
    ...
    def get_recipe_env(self, arch, **kwargs):
        ...
        # Rln fix: NDK r28c ships clang 19, which promotes
        # -Wincompatible-function-pointer-types from a warning to a hard
        # *error* by default. kivy's GL bindings (kivy/graphics/cgl_backend/
        # cgl_gl.c) assign glShaderSource — declared `const GLchar *const *`
        # in the NDK GLES headers — to a `const GLchar **` pointer, which
        # tripped that error and aborted the kivy compile. Downgrade it back
        # to a warning so kivy builds against the modern NDK.
