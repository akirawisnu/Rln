...
Usage:
  ...
  python main.py do script.do          # Run a do-file (batch mode)
...
def run_do_file(filepath: str, parser: CommandParser, console: Console):
    ...
    while i < len(commands):
        ...
        if cmd.lower().startswith("do "):
            # Expand macros in nested paths so `do "$path/child.do"` and
        ...
        try:
            ...
            # so do-files fail loudly instead of silently running to the end.
            on_err = getattr(parser.state, "on_error", "stop")
            if on_err == "stop":
...
def run_interactive(console: Console):
    ...
    while True:
        try:
            ...
            # Handle do-file command from REPL
...
def main():
    ...
    ap.add_argument("command", nargs="?", help="'do' to run a do-file, or 'gui' to launch GUI")
    ap.add_argument("dofile", nargs="?", help="Path to do-file")
