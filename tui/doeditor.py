"""
TUI Do-File Editor with other statistical tools/Rln syntax highlighting.
...
if HAS_TEXTUAL:
    ...
    class DoEditorApp(App):
        """Do-file editor with syntax highlighting."""
        ...
        def __init__(self, filepath=None, state=None, console=None):
            super().__init__()
            self.filepath = filepath or "untitled.do"
            self.state = state
        ...
        def action_new_file(self) -> None:
            editor = self.query_one("#editor", TextArea)
            editor.load_text("")
            self.filepath = "untitled.do"
            self.modified = False
            self.title = "Rln Editor - untitled.do"
            self.query_one("#status-bar", Static).update(self._status())
