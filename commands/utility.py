...
HELP_TOPICS = {
    ...
    "global": (
        ...
        "Define a global macro. Reference with $macname. Persists across scripts."),
    "foreach": (
    ...
    "set": (
        ...
        "  set on_error stop     halt scripts on error (default)\n"
    ...
    "do": (
        'do "filename.rln"',
        "Execute a script file. Supports * and // comments, /* block comments */,\n"
        "/// line continuation, nested scripts, and every scripting\n"
        "construct (local, global, foreach, forvalues, quietly, capture).\n"
        "On error the script file stops unless 'set on_error continue' was set."),
    "doedit": (
        'doedit ["filename.rln"]',
        "Open a terminal-based script editor with syntax highlighting.\n"
...
def cmd_do(rest: str, state: AppState, console: Console):
    """
    do "filename.rln"
    Execute a script file from within the REPL.
    """
    filepath = rest.strip().strip("\"'")
    if not filepath:
        console.print('[red]Syntax: do "filename.rln"[/red]')
        return
