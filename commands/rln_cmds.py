"""
Rln-specific commands: ssc (pip wrapper), doedit (do-file editor),
...
# ──────────────────────────────────────────────
#  doedit — Do-file editor with syntax highlighting
# ──────────────────────────────────────────────

def cmd_doedit(rest: str, state: AppState, console: Console):
    """
    doedit "filename.do"
    doedit                   — Open a new untitled do-file

    Opens a terminal-based do-file editor with syntax highlighting.
    """
...
def _simple_editor(filepath, state, console):
    """Basic line-based do-file editor when Textual is not available."""
    lines = []
    ...
    console.print("[dim]Type do-file commands line by line. Empty line to finish editing.[/dim]")
    console.print()
...
def _highlight_line(line: str) -> str:
    """Simple syntax highlighting for do-file lines using rich markup."""
...
def cmd_python(rest: str, state: AppState, console: Console):
    ...
    if rest == "{" or rest == "":
        # When running inside a do-file, the runner pre-collects the block
...
def _run_python_code(code, state, console):
    ...
    inside a do-file is indented for readability) still parses. This is
    ...
    loaded to begin with (Gemini Bug 9 fix), so a do-file can start with:
    ...
    # code that was indented for readability inside a do-file.
    ...
    try:
        ...
        # even if we started with no data. Without this, a do-file can't
