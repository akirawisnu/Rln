...
These are the features that make scripts feel like a real statistical package.
"""
...
def cmd_global(rest: str, state: AppState, console: Console):
    ...
    Persists across scripts (unlike local which is scoped).
    """
...
def cmd_foreach(rest: str, state: AppState, console: Console):
    ...
    NOTE: In interactive mode, collects lines until closing }.
          In scripts, the preprocessor handles braces.
    """
...
def _collect_block(state, console):
    """Collect lines until closing }. For interactive or script file mode."""
    # Check if we're in a script file (lines pre-collected via _pending_block)
